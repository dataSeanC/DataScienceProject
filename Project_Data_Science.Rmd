...
mp <- ggplot() +  mapWorld +
  geom_point(data = graph_data, mapping = aes(x=lon, y=lat, group = Country, size = total, color = "#000099"), show.legend = FALSE) +
  theme_classic() +
...
terrorism %>%
  filter(Country == "United States")  %>%
  mutate(bKilled = as.factor(ifelse(Killed > 1, 1, 0))) %>%
  na.omit() %>%
  ggplot(aes(Year, fill = Weapon_type)) +
  geom_density(alpha = 0.5) +
  scale_x_log10() +
  facet_wrap(~ bKilled, ncol = 1)
```
