---
title: 'Global Terrorism Database (GTD) using GGplot and Shiny'
...
##
The dataset included 170,350 observations.
...
# Load and Clean
```{r warning=FALSE, echo=FALSE, r warning=FALSE}
terrorism <- read_xlsx(path = "Data Science/ProjectDataScience/globalterrorismdb_0617dist.xlsx", col_types = "text") %>%
  select("Year" = iyear, 
         "Month" = imonth, 
         "Day" = iday, 
         "Country" = country_txt, 
...
ggplot(count, 
  ...
  xlab("Year") +
  ylab("Count") +
  ggtitle("Number of Terrorist Acivities Each Year Worldwide") +
...
## 
We can see that the most recent years had a significant inrease in violence.
...
##
Which teorrist group likes to target these different groups?
...
##
Which target are prefered in different countries?
...
```{r warning=FALSE, message=FALSE, warning=FALSE, r warning=FALSE}
    mp <- NULL
...
# Global Terrorist Attacks by Group
...
```{r Terroist Attacks in USA, message=FALSE, warning=FALSE}
count <- terrorism %>%
